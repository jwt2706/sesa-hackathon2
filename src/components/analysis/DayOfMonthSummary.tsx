...
const DayOfMonthSummary = ({ trades }: { trades: Trade[] }) => {
  ...
  return (
    <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-6">
      <h3 className="text-white font-bold mb-3">Most Profitable Days (by total P/L)</h3>

      {top.length === 0 ? (
        <p className="text-white/60">No trades available to analyze.</p>
      ) : (
        <div className="space-y-3">
          {top.map((t) => (
            <div key={t.day} className="flex items-center justify-between">
              <div>
                <p className="text-white font-medium">Day {t.day}</p>
                <p className="text-sm text-white/60">{t.count} trades · avg {t.avgPL.toFixed(2)}</p>
              </div>
              <div>
          ...
          <div className="mt-4 p-3 bg-white/5 rounded-lg border border-white/10 text-sm text-white/60">
            <p>
